...
Broken Links lets you check the content of your posts and pages for broken links, images, and BBCode `link`/`url`/`img` shortcodes, from a dedicated screen in the admin area. It is not automatic and does not run on a schedule or on the front end of your site — a check only ever happens when you click the "Check for broken links" button on the Check Links tab.

Results show a summary of how many items were checked, working, broken, or could not be checked, followed by a table of just the broken and unreachable ones: post type, the title of the post or page it was found in (linked to that item's edit screen), the type of link (Link, Image, BBCode Link, BBCode URL, or BBCode Image), and its status. A post type filter lets you narrow the results in real time, without reloading the page.
